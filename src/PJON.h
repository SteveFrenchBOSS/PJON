...
public:
	...
	uint16_t receive()
	{
		...
		for (uint16_t i = 0; i < length; i++)
		{
			...
			{
#ifdef DIAGNOSE
				Serial.printf("strategy.receive_frame(%x, %u)\n", data + i, length - i);
#endif
				batch_length = strategy.receive_frame(data + i, length - i);

#ifdef DIAGNOSE
				Serial.printf("PJON batch length: %u\n", batch_length);
#endif
...
#ifdef DIAGNOSE
					Serial.println("batch_length == PJON_FAIL || batch_length == 0");
#endif
					return PJON_FAIL;
				}
			}
			...
			if (i == 1)
			{
				...
				if (
					...
					((data[0] == PJON_BROADCAST) && (data[1] & PJON_ACK_REQ_BIT)) || ((data[1] & PJON_EXT_LEN_BIT) && !(data[1] & PJON_CRC_BIT)) || (!PJON_INCLUDE_PACKET_ID && (data[1] & PJON_PACKET_ID_BIT)) || (!PJON_INCLUDE_PORT && (data[1] & PJON_PORT_BIT)) || ((!PJON_INCLUDE_MAC && mac) || (mac && !(data[1] & PJON_CRC_BIT))) || (drop && !mac))
					return PJON_BUSY;
	...
	uint16_t send_packet(const uint8_t *payload, uint16_t length)
	{
		...
		{
			Serial.println("strategy.can_start() failed");
			return PJON_BUSY;
		}
#ifdef DIAGNOSE
		Serial.println("strategy.send_frame()");
#endif
		...
		uint16_t rx_port = PJON_BROADCAST)
	{
		...
		{
			Serial.println("Compose packet failed");
			return PJON_FAIL;
		}
	...
	bool known_packet_id(const PJON_Packet_Info &info)
	{
		for (uint8_t i = 0; i < PJON_MAX_RECENT_PACKET_IDS; i++)
			if (
				...
				info.tx.id == recent_packet_ids[i].sender_id && (((info.header & PJON_MODE_BIT) && (recent_packet_ids[i].header & PJON_MODE_BIT) && PJONTools::id_equality((uint8_t *)info.tx.bus_id, (uint8_t *)recent_packet_ids[i].sender_bus_id, 4)) || (!(info.header & PJON_MODE_BIT) && !(recent_packet_ids[i].header & PJON_MODE_BIT))))
				return true;
		save_packet_id(info);
		return false;
	};
