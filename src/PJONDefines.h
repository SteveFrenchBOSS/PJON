...
#else
using PJON_Receiver = std::function<void(uint8_t *payload,
                                         uint16_t length,
                                         const PJON_Packet_Info &packet_info)>;
#endif
...
struct PJONTools
{
  ...
  static uint8_t packet_overhead(uint8_t header)
  {
    return (
        (
            (header & PJON_MODE_BIT) ? (header & PJON_TX_INFO_BIT ? 11 : 6) : (header & PJON_TX_INFO_BIT ? 2 : 1)) +
        (header & PJON_EXT_LEN_BIT ? 2 : 1) + (header & PJON_CRC_BIT ? 4 : 1) + (header & PJON_PORT_BIT ? 2 : 0) + (header & PJON_PACKET_ID_BIT ? 2 : 0) + (header & PJON_MAC_BIT ? 12 : 0) + 2 // header + header's CRC
    );
  };
